...
def reference_preflop_logits(features: HandClassFeatures, state_key: StateKey) -> Dict[int, float]:
    """Closed-form reference logits used to ridge-fit :data:`HEURISTIC_BETA_PREFLOP` against ``phi``."""
...
def _ridge_fit_preflop_beta(
    ...
    for _ in range(n_samples):
        ...
        leg = reference_preflop_logits(feat, sk)
        rows.append(phi)
...
def _canonical_action(action_bucket: int) -> int:
    ...
    if action_bucket == CHECK_CALL:
        return CHECK_CALL
    if action_bucket in (RAISE, 3, 4):
        return RAISE
    raise ValueError(f"Unknown action bucket: {action_bucket!r}")
